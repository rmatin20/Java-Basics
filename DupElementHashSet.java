...
public class DupElementHashSet {
	...
	public static void main(String[] args) {
		...
		for (String eachElement :wkday) {
			...
			if(setObject.add(eachElement)==false){/*add() will see if Sun is added from the array 
												setObject. If not, then HashSet will add Sun. This 
												will go on. When it will try to add next Sun, it'll 
												see that Sun is already added, so it's a duplicate 
